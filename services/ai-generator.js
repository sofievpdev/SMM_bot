...
/**
 * Возвращает случайный профессиональный титул
 * Варианты: диетолог, терапевт-диетолог, нутрициолог, специалист по питанию и т.д.
 */
function getRandomProfessionalTitle() {
  const titles = [
    'диетолог',
    'терапевт-диетолог',
    'нутрициолог',
    ...
    'специалист по нутрициологии',
  ];
  return titles[Math.floor(Math.random() * titles.length)];
}

/**
 * Возвращает хештеги для темы дня
 * @param {string} dayTheme - Тема дня
 * @returns {string} - Строка с хештегами
 */
function getHashtagsForTheme(dayTheme) {
  const hashtagsByTheme = {
    oncology: '#онкология #профилактикарака #здоровье #медицина #превентивнаямедицина',
    nutrition: '#питание #здоровоепитание #нутрициология #диетолог #правильноепитание',
    longevity: '#долголетие #биохакинг #antiaging #здоровье #longevity',
    wellness: '#wellness #здоровье #ЗОЖ #wellbeing #образжизни',
    case_study: '#кейс #диетолог #консультация #здоровье #результат',
    weight_loss: '#похудение #снижениевеса #метаболизм #здоровоепитание #диета',
    qa_inspiration: '#мотивация #здоровье #вопросответ #ЗОЖ #вдохновение',
  };
  return hashtagsByTheme[dayTheme] || '#здоровье #медицина #диетолог #нутрициология';
}
...
export function buildFullSystemPrompt(channel) {
  ...
  if (channel.contentStyle?.writingGuidelines) {
    ...
    if (wg.useEmoji) prompt += '- Использовать эмодзи: да, но со вкусом (до 5 эмодзи на весь пост, не перебор)\n';
  ...
  if (channel.contentStyle?.writingGuidelines?.professionalism) {
    prompt += 'КОНТЕКСТ АВТОРА:\n';
    prompt += '- Сертифицированный фитнес-нутрициолог с 2015 года (более 10 лет практики)\n';
    prompt += '- Диплом по клинической диетологии Карлова университета в Праге (2018)\n';
    prompt += '- Специализация: превентивная медицина, функциональная диетология\n\n';
    ...
    prompt += `✅ Пиши как опытный ${getRandomProfessionalTitle()} (НЕ называй себя врачом!), а не как обычный человек\n`;
    prompt += '✅ Сохраняй профессиональный уровень и "вкус"\n';
    prompt += '✅ Баланс: доказательная медицина + функциональная/превентивная медицина\n';
    prompt += '✅ Статьи должны быть полноценные (800-950 символов), не короткие тизеры\n';
    prompt += '✅ Включай ссылки на исследования, если упоминаешь их\n';
    prompt += '✅ Пиши живо и по-человечески, от первого лица - как будто это пишет РЕАЛЬНЫЙ специалист\n';
    prompt += '✅ ПРОФЕССИОНАЛЬНАЯ СПОКОЙНАЯ ПОДАЧА - не дружеский пересказ, а экспертная позиция\n';
    prompt += '✅ НЕ используй ** для выделения текста - ТОЛЬКО простой текст и эмодзи\n';
    prompt += '✅ НЕ используй ## заголовки (Markdown headers) - ТОЛЬКО эмодзи для структуры\n\n';
    ...
    prompt += '❌ НЕ пиши как AI/ChatGPT - избегай шаблонных фраз и структур\n';
    prompt += '❌ НЕ используй звёздочки ** для форматирования\n';
    prompt += '❌ НЕ используй ## для заголовков - только эмодзи\n';
    prompt += '❌ НЕ злоупотребляй фразами "За X лет практики..." - используй максимум раз в неделю, варьируй начала постов\n\n';

    prompt += 'ВАЖНЫЕ НЮАНСЫ РЕКОМЕНДАЦИЙ:\n';
    prompt += '🌙 Углеводы вечером: С точки зрения превентивной медицины, вечером важно не налегать на углеводы. ';
    prompt += 'Если добавлять - то только природные и СЛОЖНЫЕ углеводы. Важно избегать фруктов и быстрых углеводов, ';
    prompt += 'чтобы не вызвать резких скачков сахара в крови перед сном.\n';
  }
  ...
  try {
    ...
    const message = await client.messages.create({
      ...
      messages: [
        {
          role: 'user',
          content: `Трансформируй этот контент в полноценный Telegram пост.

ТРЕБОВАНИЯ:
- ⚠️ ДЛИНА: МАКСИМУМ 950 символов! НЕ БОЛЬШЕ! (лимит Telegram для фото)
- ⚠️ Если превысишь 950 символов - текст обрежется и пост будет испорчен!
- Минимум 800 символов, максимум 950 - это ЖЁСТКОЕ требование
- Это должна быть полная, законченная статья, НЕ тизер
- Профессиональная спокойная подача (НЕ дружеский пересказ, а экспертная позиция)
- Пиши живо, как реальный специалист, НЕ как AI/ChatGPT
- НЕ используй форматирование ** (звёздочки) - ТОЛЬКО простой текст и эмодзи
- НЕ используй ## заголовки (Markdown headers) - ТОЛЬКО эмодзи для структуры
- Используй эмодзи со вкусом (до 5 эмодзи на весь пост, не перебор)
- Включи выводы и CTA в конце
- В КОНЦЕ ПОСТА ОБЯЗАТЕЛЬНО добавь хештеги: #здоровье #медицина #диетолог #нутрициология #ЗОЖ

КОНТЕНТ ДЛЯ ТРАНСФОРМАЦИИ:
${sourceText}`,
        },
      ],
      system: systemPrompt,
  ...
  try {
    ...
    const message = await client.messages.create({
      ...
      messages: [
        {
          role: 'user',
          content: `Создай полноценный Telegram пост на основе этой идеи.

ТРЕБОВАНИЯ:
- ⚠️ ДЛИНА: МАКСИМУМ 950 символов! НЕ БОЛЬШЕ! (лимит Telegram для фото)
- ⚠️ Если превысишь 950 символов - текст обрежется и пост будет испорчен!
- Минимум 800 символов, максимум 950 - это ЖЁСТКОЕ требование
- Полный, законченный текст от начала до конца
- Профессиональная спокойная подача (НЕ дружеский пересказ, а экспертная позиция)
- Живой и человечный стиль - НЕ как AI/ChatGPT
- НЕ используй форматирование ** (звёздочки) - ТОЛЬКО простой текст и эмодзи
- НЕ используй ## заголовки (Markdown headers) - ТОЛЬКО эмодзи для структуры
- Используй эмодзи со вкусом (до 5 эмодзи на весь пост, не перебор)
- Включи вводную часть, основной контент и выводы с CTA
- В КОНЦЕ ПОСТА ОБЯЗАТЕЛЬНО добавь хештеги: #здоровье #медицина #диетолог #нутрициология #ЗОЖ
...
${article.referenceLinks.map((link, i) => `${i + 1}. ${link}`).join('\n')}`;
    ...
    const message = await client.messages.create({
      model: 'claude-opus-4-1',
      max_tokens: 2500,  // Достаточно для полноценной статьи 900-1200 символов с выводами
      messages: [
        {
          role: 'user',
          content: `На основе следующей статьи создай полноценный авторский Telegram пост для канала о здоровье и медицине.

ТРЕБОВАНИЯ (КРИТИЧЕСКИ ВАЖНЫ!):
- ⚠️ ДЛИНА: МАКСИМУМ 950 символов! НЕ БОЛЬШЕ! (лимит Telegram для фото)
- ⚠️ Если превысишь 950 символов - текст обрежется и пост будет испорчен!
- Минимум 800 символов, максимум 950 - это ЖЁСТКОЕ требование
- ПОЛНАЯ, законченная статья, НЕ тизер
- Структура: вводная часть → основной контент → выводы → CTA
- Информация из статьи переформатирована в авторский контент
- Ссылки на исследования сохранены (если они есть)
- ПРОФЕССИОНАЛЬНАЯ СПОКОЙНАЯ ПОДАЧА (НЕ дружеский пересказ, а экспертная позиция)
- Живой и человечный стиль, как от реального специалиста - НЕ как AI/ChatGPT
- БЕЗ сленга и чрезмерно дружеского тона
- НЕ используй форматирование ** (звёздочки) - ТОЛЬКО простой текст и эмодзи
- НЕ используй ## заголовки (Markdown headers) - ТОЛЬКО эмодзи для структуры
- Используй эмодзи со вкусом (до 5 эмодзи на весь пост, не перебор)
- CTA уместный для темы "${dayTheme}"
- В КОНЦЕ ПОСТА ОБЯЗАТЕЛЬНО добавь хештеги: ${getHashtagsForTheme(dayTheme)}
