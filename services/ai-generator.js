 ...
 * @param {number} maxLength - Максимальная длина (по умолчанию 4096 для Telegram, 2048 если с картинкой)
...
function trimTextAtWordBoundary(text, maxLength = 4096) {
  ...
  // 1. СНАЧАЛА ищем последнюю ПОЛНУЮ пунктуацию (. ! ?)
  ...
  // 2. Если нашли пунктуацию в тексте - используем её
  if (lastPeriodIndex > 0) {
    // Обрезаем сразу после пунктуации
    trimmed = trimmed.substring(0, lastPeriodIndex + 1);
  } else {
    // 3. Если пунктуации нет - обрезаем на последнем полном слове
  ...
  // 4. Удаляем незаконченную разметку в конце (**, #, лишние эмодзи и т.д.)
  trimmed = trimmed
    .replace(/\*\*\s*$/, '') // Удалить ** в конце
    .replace(/#+\s*$/, '')   // Удалить # в конце
    .replace(/💡\s*\*\*\s*$/, '') // Удалить неполный CTA с эмодзи
    .replace(/[✓✅❌⚠️ℹ️]\s*$/, '') // Удалить одиночные эмодзи в конце
    .trim();
